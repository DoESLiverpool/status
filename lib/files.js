...
function jsonRequest(url){
    var res = request('GET', url, { headers: { 'User-agent': exports.cfg['settings']['user-agent'], 'Authorization': 'token ' + process.env.GIT}});
...
function xivelyFeed(obj, setup) {
    if (process.env.XIVELY == null) {
        console.error('No Xively api key specified!');
        throw "No key";
    }
    if (setup) {
        setInterval(function () {
            var xively = jsonRequest(obj['xively-endpoint'] + '?key=' + process.env.XIVELY);
