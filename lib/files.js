    ...
    if(setup){
        ...
        setInterval(function(){
            if(exports.post[obj['obj_name']]['last-post'] == null){
                ...
                if(obj['alttext']){
                    sockets.update(obj['obj_name'], 'Broken', false, strftime('$a %d %m, %Y %H:%M', exports.post[obj['obj_name']]['offline_date']));
                }
                else{
                    sockets.update(obj['obj_name'], 'Offline', false, strftime('$a %d %m, %Y %H:%M', exports.post[obj['obj_name']]['offline_date']));
                }
            }
            if (exports.post[obj['obj_name']]['last-post'] >= new Date().getTime() - (obj['freq'] * 60000)){
                ...
                if(obj['alttext']){
                    sockets.update(obj['obj_name'], 'Working', true, strftime('$a %d %m, %Y %H:%M'));
                }
                else{
                    sockets.update(obj['obj_name'], 'Online', true, strftime('$a %d %m, %Y %H:%M'));
                }
            }
        }, obj['freq'] * 60000);
    }
