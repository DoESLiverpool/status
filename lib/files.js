...
function gitStatus(obj, setup){
    ...
    if(!isEmpty(json)){
        ...
        if(obj['alttext']){
            sockets.update(obj['obj_name'], 'Working', false, obj['offline_date']);
        }
        else{
            sockets.update(obj['obj_name'], 'Online', false, obj['offline_date']);
        }
    }
    if(setup){
...
function wordGet(obj, setup){
    ...
    if(res['statusCode'] >= 300){
        ...
        obj['offline_date'] = date('%a %d %B, %H:%M');
        if(obj['alttext']){
            sockets.update(obj['obj_name'], 'Working', true, obj['offline_date']);
        }
        else{
            sockets.update(obj['obj_name'], 'Online', true, obj['offline_date']);
        }
    }
    if(setup) {
        setInterval(function () {
    ...
    if(setup){
        ...
        setInterval(function(){
            if(exports.post[obj['obj_name']]['last-post'] == null){
                obj['offline'] = true;
                obj['offline_date'] = date('$a %d %m, %Y %H:%M', exports.post[obj['obj_name']]['last-post']);
                if(obj['alttext']){
                    sockets.update(obj['obj_name'], 'Broken', false, date('$a %d %m, %Y %H:%M', exports.post[obj['obj_name']]));
                }
                else{
                    sockets.update(obj['obj_name'], 'Offline', false, date('$a %d %m, %Y %H:%M', exports.post[obj['obj_name']]));
                }
            }
            if (exports.post[obj['obj_name']]['last-post'] >= new Date().getTime() - (obj['freq'] * 60000)){
                ...
                if(obj['alttext']){
                    sockets.update(obj['obj_name'], 'Working', true, date('$a %d %m, %Y %H:%M'));
                }
                else{
                    sockets.update(obj['obj_name'], 'Online', true, date('$a %d %m, %Y %H:%M'));
                }
            }
        }, obj['freq'] * 60000);
    }
...
function xivelyFeed(obj, setup) {
    ...
    if (setup) {
        setInterval(function () {
            ...
            sockets.update('xively-'+obj['obj_name'], xively['current_value'], null, date('$a %d %m, %Y %H:%M'));
